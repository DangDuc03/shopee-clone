...
export default function Cart() {
  ...
  const isAllChecked = useMemo(() => extendPurchases.every((data) => data.checked), [extendPurchases])
  const purchasesChecked = useMemo(() => extendPurchases.filter((data) => data.checked), [extendPurchases])
  const countPurchasesChecked = purchasesChecked.length
  const totalPricePurchasesChecked = useMemo(
    () =>
      purchasesChecked.reduce((total, purchase) => {
        return total + purchase.product.price * purchase.buy_count
      }, 0),
    [purchasesChecked]
  )
  const totalPriceSavingPurchasesChecked = useMemo(
    () =>
      purchasesChecked.reduce((total, purchase) => {
  ...
  const isChoosePruchaseIdFromLocation = (location.state as { purchaseId?: string } | null)?.purchaseId
    ...
    setExtendPurchases((prev) => {
      ...
      return (
        listProductIncart?.map((purchase) => {
          const isChoosePurchase = isChoosePruchaseIdFromLocation === purchase._id
          return {
            ...purchase,
            distable: false,
            checked: isChoosePurchase || Boolean(extendPurcahsesOject[purchase._id]?.checked)
          }
        }) || []
      )
    })
  }, [listProductIncart, isChoosePruchaseIdFromLocation])

  // clear state from location when component unmounts
  useEffect(() => {
    return () => {
          ...
          <div className='sticky bottom-0 z-10 mt-5 flex flex-col overflow-auto rounded-sm bg-white p-5 shadow sm:mt-0 sm:flex-row sm:items-center'>
            ...
            <div className='my-5 flex flex-col sm:ml-auto sm:justify-center'>
              <div className='flex items-center sm:justify-end'>
                ...
                <span className='ml-2 text-customOrange md:text-2xl'>
                  ₫ {formatCurrency(totalPricePurchasesChecked)}
                </span>
              </div>
        ...
        <div className='custom-container'>
          <div className='flex flex-col items-center justify-center'>
            <div className='max-h-[400px] max-w-[400px]'>
              <img src={emptyCart} alt='empty product' />
            </div>
            <Link
              to={path.home}
              className='rounded-sm bg-customOrange px-10 py-2 uppercase text-white hover:bg-customOrange/80 md:ml-3'
            >
              Mua ngay
            </Link>
          </div>
